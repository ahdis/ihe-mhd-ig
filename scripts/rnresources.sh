cd ../resources/StructureDefinition
mv IHE.MHD.DocumentManifest.structuredefinition.xml IHE.MHD.DocumentManifest.xml			I
mv IHE.MHD.Query.Minimal.DocumentReference.structuredefinition.xml IHE.MHD.Query.Minimal.DocumentReference.xml
mv IHE.MHD.ITI-65.structuredefinition.xml IHE.MHD.ITI-65.xml						
mv IHE.mCSD.HealthcareService.structuredefinition.xml IHE.mCSD.HealthcareService.xml
mv IHE.MHD.List.structuredefinition.xml IHE.MHD.List.xml							
mv IHE.mCSD.Location.Distance.structuredefinition.xml IHE.mCSD.Location.Distance.xml
mv IHE.MHD.Provide.Comprehensive.DocumentReference.structuredefinition.xml IHE.MHD.Provide.Comprehensive.DocumentReference.xml		
mv IHE.mCSD.Location.structuredefinition.xml IHE.mCSD.Location.xml
mv IHE.MHD.Provide.Minimal.DocumentReference.structuredefinition.xml IHE.MHD.Provide.Minimal.DocumentReference.xml		
mv IHE.mCSD.Organization.structuredefinition.xml IHE.mCSD.Organization.xml
mv IHE.MHD.ProvideDocumentBundle.Comprehensive.structuredefinition.xml IHE.MHD.ProvideDocumentBundle.Comprehensive.xml		
mv IHE.mCSD.Practitioner.structuredefinition.xml IHE.mCSD.Practitioner.xml
mv IHE.MHD.ProvideDocumentBundle.Minimal.structuredefinition.xml IHE.MHD.ProvideDocumentBundle.Minimal.xml			
mv IHE.mCSD.PractitionerRole.structuredefinition.xml IHE.mCSD.PractitionerRole.xml
mv IHE.MHD.Query.Comprehensive.DocumentReference.structuredefinition.xml IHE.MHD.Query.Comprehensive.DocumentReference.xml
cd ../CapabilityStatement
mv IHE.MHD.DocumentConsumer.capabilitystatement.xml IHE.MHD.DocumentConsumer.xml
mv IHE.MHD.DocumentRecipient.capabilitystatement.xml IHE.MHD.DocumentRecipient.xml
mv IHE.MHD.DocumentResponder.capabilitystatement.xml IHE.MHD.DocumentResponder.xml
mv IHE.MHD.DocumentSource.capabilitystatement.xml IHE.MHD.DocumentSource.xml
mv IHE.PDQm.consumer.capabilitystatement.xml IHE.PDQm.consumer.xml
mv IHE.PDQm.supplier.capabilitystatement.xml IHE.PDQm.supplier..xml
mv IHE.mCSD.CareServicesSelectiveConsumer.HealtcareService.capabilitystatement.xml IHE.mCSD.CareServicesSelectiveConsumer.HealtcareService.xml
mv IHE.mCSD.CareServicesSelectiveConsumer.Location.capabilitystatement.xml IHE.mCSD.CareServicesSelectiveConsumer.Location.xml
mv IHE.mCSD.CareServicesSelectiveConsumer.Organization.capabilitystatement.xml IHE.mCSD.CareServicesSelectiveConsumer.Organization.xml
mv IHE.mCSD.CareServicesSelectiveConsumer.Practitioner.capabilitystatement.xml IHE.mCSD.CareServicesSelectiveConsumer.Practitioner.xml
mv IHE.mCSD.CareServicesSelectiveSupplier.HealtcareService.capabilitystatement.xml IHE.mCSD.CareServicesSelectiveSupplier.HealtcareService.xml
mv IHE.mCSD.CareServicesSelectiveSupplier.Location.capabilitystatement.xml IHE.mCSD.CareServicesSelectiveSupplier.Location.xml
mv IHE.mCSD.CareServicesSelectiveSupplier.Organization.capabilitystatement.xml IHE.mCSD.CareServicesSelectiveSupplier.Organization.xml
mv IHE.mCSD.CareServicesSelectiveSupplier.Practitioner.capabilitystatement.xml IHE.mCSD.CareServicesSelectiveSupplier.Practitioner.xml
mv IHE.mCSD.CareServicesUpdateConsumer.HealthcareService.capabilitystatement.xml IHE.mCSD.CareServicesUpdateConsumer.HealthcareService.xml
mv IHE.mCSD.CareServicesUpdateConsumer.Location.capabilitystatement.xml IHE.mCSD.CareServicesUpdateConsumer.Location.xml
mv IHE.mCSD.CareServicesUpdateConsumer.Organization.capabilitystatement.xml IHE.mCSD.CareServicesUpdateConsumer.Organization.xml
mv IHE.mCSD.CareServicesUpdateConsumer.Practitioner.capabilitystatement.xml IHE.mCSD.CareServicesUpdateConsumer.Practitioner.xml
mv IHE.mCSD.CareServicesUpdateSupplier.HealthcareService.capabilitystatement.xml IHE.mCSD.CareServicesUpdateSupplier.HealthcareService.xml
mv IHE.mCSD.CareServicesUpdateSupplier.Location.capabilitystatement.xml IHE.mCSD.CareServicesUpdateSupplier.HealthcareService.xml
mv IHE.mCSD.CareServicesUpdateSupplier.Organization.capabilitystatement.xml IHE.mCSD.CareServicesUpdateSupplier.Organization.xml
mv IHE.mCSD.CareServicesUpdateSupplier.Practitioner.capabilitystatement.xml IHE.mCSD.CareServicesUpdateSupplier.Practitioner.xml
cd ..
cd CodeSystem
mv IHE.formatcode.codesystem.xml IHE.formatcode.cs.xml
cd ..
cd ValueSet
mv IHE.formatcode.valueset.xml IHE.formatcode.vs.xml
cd ..
cd ImplementationGuide
mv IHE.MHD.implementationguide.xml IHE.MHD.xml			
mv IHE.PDQm.implementationguide.xml IHE.PDQm.xml		
mv IHE.mCSD.implementationguide.xml IHE.mCSD.xml
cd ..
cd ..
cd scripts